...
import VectorAvatar, { isVectorAvatar } from './avatar/VectorAvatars';
...
export default function Avatar({ nome, ruolo, size = 52, config }: { nome: string; ruolo?: string; size?: number; config?: ProfileConfigLite }) {
  ...
  const hasCfg = !!config;
  // avatar vettoriale: ha priorità e disegna il proprio cerchio (gradiente)
  if (config && (config as any).vector && isVectorAvatar((config as any).vector)) {
    return (
      <View style={{ width: size, height: size }}>
        <VectorAvatar id={(config as any).vector} size={size} color={config!.color || '#0A84FF'} />
        {badge ? (
          <View style={[styles.badge, { backgroundColor: badge.color, borderColor: DS.color.bg }]}><Text style={styles.badgeTxt}>{badge.label}</Text></View>
        ) : null}
      </View>
    );
  }
