...
function DatePickerSheet({ visible, title, day, month, onConfirm, onClose }: {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      ...
      <View style={[pk.wrap, { paddingBottom: insets.bottom + 16, backgroundColor: colors.cardEco, borderColor: colors.borderEco }]}>
        <Text style={[pk.title, { color: colors.textPrimary }]}>{title}</Text>
        <View style={pk.cols}>
          <ScrollView style={pk.col} showsVerticalScrollIndicator={false}>
            {Array.from({ length: maxD }, (_, i) => i + 1).map((n) => (
              <Pressable key={n} onPress={() => setD(n)} style={[pk.opt, n === d && { backgroundColor: 'rgba(88,204,255,0.15)' }]}>
                <Text style={[pk.optTxt, { color: n === d ? colors.blue : colors.textPrimary }]}>{pad(n)}</Text>
              </Pressable>
            ))}
          </ScrollView>
          <ScrollView style={pk.col} showsVerticalScrollIndicator={false}>
            {months.map((mn, i) => (
              <Pressable key={mn} onPress={() => setM(i + 1)} style={[pk.opt, i + 1 === m && { backgroundColor: 'rgba(88,204,255,0.15)' }]}>
                <Text style={[pk.optTxt, { color: i + 1 === m ? colors.blue : colors.textPrimary }]}>{mn}</Text>
              </Pressable>
            ))}
          </ScrollView>
        </View>
...
function Stepper({ value, onDelta, colors }: { value: number; onDelta: (d: number) => void; colors: any }) {
  return (
    <View style={st.row}>
      <Pressable hitSlop={8} onPress={() => onDelta(-1)} style={[st.btn, { backgroundColor: colors.cardEco }]}><Icon name="remove" size={18} color={colors.blue} /></Pressable>
      <Text style={[st.val, { color: colors.textPrimary }]}>{pad(value)}</Text>
      <Pressable hitSlop={8} onPress={() => onDelta(1)} style={[st.btn, { backgroundColor: colors.cardEco }]}><Icon name="add" size={18} color={colors.blue} /></Pressable>
    </View>
  );
}
...
function Section({ open, onToggle, label, children, colors }: { open: boolean; onToggle: () => void; label: string; children: React.ReactNode; colors: any }) {
  ...
  return (
    <View style={[sc.box, { backgroundColor: colors.cardEco, borderColor: open ? colors.blue : colors.borderEco }]}>
      <Pressable onPress={onToggle} style={sc.head}>
        <View style={[sc.radio, { borderColor: open ? colors.blue : colors.textDisabled }]}>{open ? <View style={[sc.dot, { backgroundColor: colors.blue }]} /> : null}</View>
        ...
        <Animated.View style={{ transform: [{ rotate: rot.interpolate({ inputRange: [0, 1], outputRange: ['0deg', '180deg'] }) }] }}>
          <Icon name="chevron-down" size={20} color={colors.textDisabled} />
        </Animated.View>
      </Pressable>
    ...
    <View style={[styles.root, { backgroundColor: colors.bgEco }]}>
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <View style={{ flex: 1, marginHorizontal: 12 }}>
          <Text style={[styles.hTitle, { color: colors.textPrimary }]}>Matrici stagionali</Text>
          <Text style={[styles.hSub, { color: colors.textDisabled }]}>Assegna una matrice e un periodo a ogni stagione.</Text>
        </View>
        <CloseButton />
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: insets.bottom + 100 }} showsVerticalScrollIndicator={false}>
          ...
          return (
            <View key={s.key} style={{ marginTop: 24 }}>
              <Section open={isOpen} onToggle={() => setOpenSeason(isOpen ? '' : s.key)} label={s.label} colors={colors}>
                <Text style={[styles.blockTitle, { color: colors.textPrimary }]}>Periodo</Text>
                <View style={styles.dateRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.fieldLabel, { color: colors.textDisabled }]}>Inizio</Text>
                    <Pressable onPress={() => setPicker({ season: s.key, which: 'start' })} style={[styles.dateField, { backgroundColor: colors.cardEco, borderColor: colors.borderEco }]}>
                      <Text style={[styles.dateTxt, { color: colors.textPrimary }]}>{pad(r.startDay)}/{pad(r.startMonth)}</Text>
                      <Icon name="calendar-outline" size={18} color={colors.textDisabled} />
                    </Pressable>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.fieldLabel, { color: colors.textDisabled }]}>Fine</Text>
                    <Pressable onPress={() => setPicker({ season: s.key, which: 'end' })} style={[styles.dateField, { backgroundColor: colors.cardEco, borderColor: colors.borderEco }]}>
                      <Text style={[styles.dateTxt, { color: colors.textPrimary }]}>{pad(r.endDay)}/{pad(r.endMonth)}</Text>
                      <Icon name="calendar-outline" size={18} color={colors.textDisabled} />
                    </Pressable>
                  </View>
                </View>

                <Text style={[styles.blockTitle, { color: colors.textPrimary, marginTop: 24 }]}>Configurazione operativa</Text>
                <View style={styles.grid}>
                  {([['Settori M', 'M'], ['Settori P', 'P'], ['Settori N', 'N']] as Array<[string, 'M' | 'P' | 'N']>).map(([lab, t]) => (
                    <View key={t} style={[styles.opCard, { backgroundColor: colors.cardEco }]}>
                      <Text style={[styles.opLabel, { color: colors.textDisabled }]}>{lab}</Text>
                      <Stepper value={(op.settori || {})[t] || 0} onDelta={(d) => setOpSet(s.key, t, d)} colors={colors} />
                    </View>
                  ))}
                  <View style={[styles.opCard, { backgroundColor: colors.cardEco }]}>
                    <Text style={[styles.opLabel, { color: colors.textDisabled }]}>Settori chiusi</Text>
                    <Stepper value={op.settoriChiusi || 0} onDelta={(d) => setOp(s.key, { settoriChiusi: Math.max(0, (op.settoriChiusi || 0) + d) })} colors={colors} />
                  </View>
                  <View style={[styles.opCard, { backgroundColor: colors.cardEco }]}>
                    <Text style={[styles.opLabel, { color: colors.textDisabled }]}>Posti letto</Text>
                    <Stepper value={op.postiLetto || 0} onDelta={(d) => setOp(s.key, { postiLetto: Math.max(0, (op.postiLetto || 0) + d) })} colors={colors} />
                  </View>
                  <View style={[styles.opCard, { backgroundColor: colors.cardEco }]}>
                    <Text style={[styles.opLabel, { color: colors.textDisabled }]}>Copertura min %</Text>
                  ...
                  <View style={[styles.opCard, { backgroundColor: colors.cardEco }]}>
                    <Text style={[styles.opLabel, { color: colors.textDisabled }]}>Personale min</Text>
                    <Stepper value={op.personaleMin || 0} onDelta={(d) => setOp(s.key, { personaleMin: Math.max(0, (op.personaleMin || 0) + d) })} colors={colors} />
                  </View>
                </View>

                <Text style={[styles.blockTitle, { color: colors.textPrimary, marginTop: 24 }]}>Matrice</Text>
                <View style={styles.matGrid}>
                  {matOptions.map((m) => {
                    const sel = r.matrice === m.id;
                    return (
                      <Pressable key={m.id} onPress={() => setRange(s.key, { matrice: m.id })} style={[styles.matCard, { backgroundColor: sel ? 'rgba(88,204,255,0.15)' : colors.cardEco, borderColor: sel ? colors.blue : 'transparent' }]}>
                        <Text style={[styles.matTitle, { color: sel ? colors.blue : colors.textPrimary }]} numberOfLines={1}>{m.label}</Text>
                        <Text style={[styles.matSub, { color: colors.textDisabled }]}>ciclo {m.durata}</Text>
                      </Pressable>
                    );
                  })}
                  <Pressable onPress={() => router.push({ pathname: '/matrice-editor', params: { season: s.key } } as any)} style={[styles.matCard, styles.matAdd, { borderColor: colors.borderEco }]}>
                    <Text style={[styles.matTitle, { color: colors.textSecondary }]}>＋ Personalizzata</Text>
                    <Text style={[styles.matSub, { color: colors.textDisabled }]}>crea sequenza</Text>
                  </Pressable>
                </View>
              </Section>
            </View>
        ...
        <View style={[styles.info, { backgroundColor: colors.cardEco, borderColor: colors.borderEco }]}>
          <Icon name="information-circle" size={22} color={colors.blue} />
          <Text style={[styles.infoTxt, { color: colors.textDisabled }]}>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 16, backgroundColor: colors.bgEco, borderTopColor: colors.borderEco }]}>
        <Pressable onPress={salva} style={[styles.save, { backgroundColor: validation.ok ? colors.blue : colors.cardEco }]}>
          <Icon name="save-outline" size={20} color={validation.ok ? '#FFFFFF' : colors.textDisabled} />
          <Text style={[styles.saveTxt, { color: validation.ok ? '#FFFFFF' : colors.textDisabled }]}>Salva configurazione stagionale</Text>
        </Pressable>
      </View>
