  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bgEco, paddingBottom: insets.bottom }]}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        ...
        <Card>
          {!staff.length ? (
            <Text style={[styles.hint, { color: colors.textDisabled }]}>Aggiungi prima del personale.</Text>
          ) : (
            <>
              <Text style={[styles.label, { color: colors.textPrimary }]}>Operatore</Text>
              ...
              <Text style={[styles.label, { color: colors.textPrimary, marginTop: 14 }]}>Tipologia</Text>
              ...
              <Text style={[styles.label, { color: colors.textPrimary, marginTop: 14 }]}>Priorità</Text>
              <View style={styles.chips}>
                {PRIORITA.map((p) => (
              ...
              <Text style={[styles.label, { color: colors.textPrimary, marginTop: 14 }]}>Periodo</Text>
              <View style={styles.chips}>
              ...
              <View style={styles.dateRow}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.subLabel, { color: colors.textSecondary }]}>{interval ? 'Dal' : 'Data'}</Text>
                  <TextInput
                    value={dateStart} onChangeText={setDateStart} placeholder="AAAA-MM-GG" placeholderTextColor={colors.textDisabled}
                    autoCapitalize="none" style={[styles.input, { color: colors.textPrimary, borderColor: colors.borderEco, backgroundColor: colors.glass }]}
                  />
                </View>
                {interval ? (
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.subLabel, { color: colors.textSecondary }]}>Al</Text>
                    <TextInput
                      value={dateEnd} onChangeText={setDateEnd} placeholder="AAAA-MM-GG" placeholderTextColor={colors.textDisabled}
                      autoCapitalize="none" style={[styles.input, { color: colors.textPrimary, borderColor: colors.borderEco, backgroundColor: colors.glass }]}
                    />
                  </View>
                ) : null}
          ...
          sorted.map((d) => (
            <Card key={d.id}>
              <View style={styles.itemRow}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.itemName, { color: colors.textPrimary }]}>{nameOf(d.infId)}</Text>
                  <Text style={[styles.itemMeta, { color: colors.textSecondary }]}>
                    {TIPO_LABEL[d.tipo]} · {d.dateEnd && d.dateEnd !== d.dateStart ? `${d.dateStart} → ${d.dateEnd}` : d.dateStart}
                  </Text>
                  <Text style={[styles.prioTag, {
                    color: d.priorita === 'alta' ? colors.danger2 : d.priorita === 'media' ? colors.warning : colors.textDisabled,
                ...
                <Pressable hitSlop={8} style={styles.iconBtn} onPress={() => del(d)}>
                  <Icon name="trash-outline" size={20} color={colors.danger2} />
                </Pressable>
              </View>
            </Card>
          ))
