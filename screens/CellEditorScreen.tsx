...
export default function CellEditorScreen() {
  ...
  if (!inf) {
    return (
      <View style={[styles.container, { backgroundColor: colors.cardEco }]}>
        <SheetHeader title="Turno" onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
        <Text style={{ color: colors.textSecondary, padding: 16 }}>Operatore non trovato.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.cardEco, paddingBottom: insets.bottom }]}>
      <SheetHeader title={inf.nome} subtitle={`${day} ${MONTHS[month]} ${year} · ${inf.qualifica}`} onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
      <ScrollView contentContainerStyle={styles.body}>
        {existing && existing.turno === 'F' ? (
          <View style={[styles.assBox, { backgroundColor: ASS_SOFT }]}>
            ...
            <Text style={[styles.assHint, { color: colors.textDisabled }]}>Le assenze si gestiscono da “Aggiungi / Modifica assenza”.</Text>
          </View>
        ) : null}
        <Text style={[styles.label, { color: colors.textSecondary }]}>TURNO</Text>
          ...
          {(['M', 'P', 'N', 'R', 'F'] as Turno[]).map((t) => {
            ...
            return (
              <Pressable
                ...
                disabled={dis}
                onPress={() => pickTurno(t)}
                style={[styles.turnBtn, { borderColor: sel ? colors.blue : colors.borderEco, backgroundColor: sel ? 'rgba(88,204,255,0.15)' : colors.cardEco, opacity: dis ? 0.35 : 1 }]}
              >
                <View style={[styles.turnSquare, { backgroundColor: SHIFT_C[t] + '33' }]}>
                  <Text style={[styles.turnLetter, { color: SHIFT_C[t] }]}>{t}</Text>
                </View>
                <Text style={[styles.turnName, { color: colors.textSecondary }]}>{TURNI[t].label}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        {isWork(turno) ? (
          <>
            <Text style={[styles.label, { color: colors.textSecondary }]}>REPARTO</Text>
            {compat.length === 0 ? (
              <View style={[styles.warn, { backgroundColor: 'rgba(255,107,107,0.15)' }]}>
                <Text style={[styles.warnTxt, { color: colors.danger2 }]}>Nessun reparto di competenza ha settori per il turno {TURNI[turno].label}.</Text>
              </View>
            ) : null}
            {compat.map((r) => {
              ...
              return (
                <Pressable
                  ...
                  style={[styles.repCard, { borderColor: sel ? colors.blue : colors.borderEco, backgroundColor: sel ? 'rgba(88,204,255,0.15)' : colors.cardEco, opacity: dis ? 0.4 : 1 }]}
                >
                  <View style={[styles.repIcon, { backgroundColor: avatarColor(gi) }]}>
                    <Text style={styles.repIconTxt}>{r.sigla}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.repName, { color: colors.textPrimary }]}>{r.nome}</Text>
                    <Text style={[styles.repSub, { color: colors.textSecondary }]}>
            ...
            {repartoId ? (
              <>
                <Text style={[styles.label, { color: colors.textSecondary }]}>SETTORE</Text>
                  ...
                  {Array.from({ length: getRep(reparti, repartoId)?.settori[turno as 'M' | 'P' | 'N'] || 0 }).map((_, i) => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={[styles.secBtn, { borderColor: sel ? colors.blue : colors.borderEco, backgroundColor: sel ? 'rgba(88,204,255,0.15)' : colors.cardEco, opacity: occ || esente ? 0.55 : 1 }]}
                      >
                        <Text style={[styles.secCode, { color: sel ? colors.blue : colors.textPrimary }]}>{code}</Text>
                        {occ ? <Text style={[styles.secMeta, { color: colors.textDisabled }]}>{occ}</Text> : esente ? <Text style={[styles.secMeta, { color: colors.textDisabled }]}>esente</Text> : null}
                      </Pressable>
                    );
                  })}
                </View>
            ...
            {warn ? (
              <View style={[styles.warn, { backgroundColor: 'rgba(255,107,107,0.15)' }]}>
                <Text style={[styles.warnTxt, { color: colors.danger2 }]}>⚠ Riposo &lt;11h ({warn.side}): fine {warn.a} → inizio {warn.b}. Modifica vietata dalla regola CCNL art. 26.</Text>
              </View>
            ) : null}
          </>
        ) : null}
      ...
      <View style={[styles.foot, { borderTopColor: colors.borderEco }]}>
        <Button title="Annulla" variant="secondary" full onPress={() => (router.canGoBack() ? router.back() : router.replace("/"))} style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  assMot: { fontSize: 15, fontWeight: '700' },
  assHint: { fontSize: 11, marginTop: 4 },
  container: { flex: 1 },
  ...
  turnName: { fontSize: 11, fontWeight: '600' },
  ...
  repName: { fontSize: 13, fontWeight: '700' },
  repSub: { fontSize: 11, marginTop: 1 },
  ...
  secMeta: { fontSize: 11, fontWeight: '600', marginTop: 2 },
  warn: { borderRadius: 10, padding: 11, marginTop: 10 },
  warnTxt: { fontSize: 12, fontWeight: '600' },
