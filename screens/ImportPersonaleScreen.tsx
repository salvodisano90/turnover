    ...
    <View style={[styles.root, { backgroundColor: colors.bgEco }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 40 }} showsVerticalScrollIndicator={false}>
        <GlassCard style={{ marginBottom: 12 }}>
          <Text style={[styles.note, { color: colors.textSecondary }]}>Colonne: {STAFF_IMPORT_COLUMNS.join(', ')}.</Text>
          <Text style={[styles.note, { color: colors.textDisabled, marginTop: 6 }]}>La selezione diretta del file .xlsx sarà disponibile con il modulo file. Per ora scarica il template, compilalo e incolla qui le righe (separate da «;» o tab).</Text>
        ...
        <TextInput
          style={[styles.area, { color: colors.textPrimary, borderColor: colors.borderEco, backgroundColor: colors.cardEco }]}
          placeholder={'Mario;Rossi;12345;Infermiere;FT36;36;Medicina;weekend;notti'}
          placeholderTextColor={colors.textDisabled} multiline value={raw} onChangeText={setRaw}
        />
        ...
        {result ? (
          <>
            ...
            <GlassCard>
              ...
              {result.errors.length ? <Text style={[styles.sum, { color: colors.danger2 }]}>✗ {result.errors.length} errori</Text> : null}
              {result.duplicates.length ? <Text style={[styles.sum, { color: colors.warning }]}>⚠ {result.duplicates.length} duplicati</Text> : null}
            </GlassCard>
            {result.valid.slice(0, 30).map((d, i) => (
              <GlassCard key={i} style={styles.prev}><Text style={[styles.pNome, { color: colors.textPrimary }]}>{d.cognome ? `${d.nome} ${d.cognome}` : d.nome}</Text><Text style={[styles.pMeta, { color: colors.textDisabled }]}>{d.qualifica} · {d.contratto}{d.reparti.length ? ` · ${d.reparti.length} reparto/i` : ''}</Text></GlassCard>
            ))}
            {result.errors.slice(0, 20).map((e, i) => <Text key={`e${i}`} style={[styles.err, { color: colors.danger2 }]}>Riga {e.row} · {e.field}: {e.message}</Text>)}
            {result.duplicates.slice(0, 20).map((e, i) => <Text key={`d${i}`} style={[styles.err, { color: colors.warning }]}>Riga {e.row}: {e.reason}</Text>)}
