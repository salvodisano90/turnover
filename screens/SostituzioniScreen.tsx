...
export default function SostituzioniScreen() {
  ...
  const scoreColor = (s: number) => (s >= 85 ? colors.green : s >= 70 ? colors.warning : colors.danger2);

  return (
    <View style={[styles.container, { backgroundColor: colors.bgEco, paddingBottom: insets.bottom }]}>
      <SheetHeader title="Sostituzioni" subtitle={`${rep ? rep.nome : ''} · giorno ${day}`} onClose={() => (router.canGoBack() ? router.back() : router.replace("/"))} />
            ...
            {ranks.map((r, i) => {
              ...
              return (
                <Card key={i} style={{ marginBottom: 12 }}>
                  <Text style={[styles.tHead, { color: colors.textPrimary }]}>
                    {TURNI[r.target.turno]?.label || r.target.turno} · {r.target.settore}
                  </Text>
                  {eligibles.length === 0 ? (
                    <Text style={[styles.none, { color: colors.textDisabled }]}>Nessun sostituto idoneo (riposo 11h / disponibilità / compatibilità).</Text>
                  ) : eligibles.map((c, j) => (
                    <View key={c.infId} style={[styles.cand, { borderTopColor: colors.borderEco, borderTopWidth: j === 0 ? 0 : StyleSheet.hairlineWidth }]}>
                      <View style={styles.candTop}>
                        <Text style={[styles.rank, { color: colors.textSecondary }]}>{j + 1}°</Text>
                        <Text style={[styles.cName, { color: colors.textPrimary }]} numberOfLines={1}>{c.nome}</Text>
                        <View style={[styles.badge, { backgroundColor: scoreColor(c.score) }]}>
                          <Text style={styles.badgeTxt}>{c.score}%</Text>
                        </View>
                      </View>
                      <Text style={[styles.motivo, { color: colors.textDisabled }]}>{c.motivo}</Text>
                      <Button title="Assegna" variant="soft" small onPress={() => assign(r.target, c.infId, c.nome)} style={{ alignSelf: 'flex-start', marginTop: 6 }} />
                    </View>
                  ))}
                  {scartati > 0 ? <Text style={[styles.scartati, { color: colors.textDisabled }]}>{scartati} operatori non proponibili (occupati, assenti o non compatibili).</Text> : null}
                </Card>
              );
            })}
          </>
      ...
      <View style={[styles.foot, { borderTopColor: colors.borderEco }]}>
        ...
        <Button title="Chiudi" variant="primary" onPress={() => (router.canGoBack() ? router.back() : router.replace("/"))} style={{ flex: 1 }} />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  empty: { fontSize: 13, fontWeight: '700', textAlign: 'center', paddingVertical: 6 },
  tHead: { fontSize: 13, fontWeight: '800', marginBottom: 6 },
  none: { fontSize: 12, lineHeight: 18 },
  cand: { paddingVertical: 9 },
  candTop: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  rank: { fontSize: 13, fontWeight: '800', width: 24 },
  cName: { flex: 1, fontSize: 15, fontWeight: '700' },
  ...
  scartati: { fontSize: 11, marginTop: 8, fontStyle: 'italic' },
