...
// Export Inverse: matrice originale + inversa di un operatore su un periodo, con statistiche.
// Riusa XLSX (xlsx-js-style) e lo stesso meccanismo web/native di exportPianoXLSX.
export async function exportInverseXLSX(params: {
  opName: string; year: number; month: number; fromDay: number; toDay: number;
  days: number[]; source: string[]; inverse: string[];
}): Promise<void> {
  const { opName, year, month, fromDay, toDay, days, source, inverse } = params;
  const MM = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre'];
  const pad = (n: number) => String(n).padStart(2, '0');
  const periodo = `dal ${pad(fromDay)}/${pad(month + 1)}/${year} al ${pad(toDay)}/${pad(month + 1)}/${year}`;
  const count = (arr: string[]) => arr.reduce((m: Record<string, number>, t) => { m[t] = (m[t] || 0) + 1; return m; }, {});
  const cIn = count(inverse);
  const aoa: any[][] = [
    ['Inverse — Matrice complementare'],
    ['Operatore', opName],
    ['Mese', `${MM[month]} ${year}`],
    ['Periodo', periodo],
    ['Giorni', days.length],
    [],
    ['Giorno', ...days],
    ['Matrice originale', ...source],
    ['Matrice inversa', ...inverse],
    [],
    ['Statistiche (matrice inversa)'],
    ['Mattina (M)', cIn['M'] || 0],
    ['Pomeriggio (P)', cIn['P'] || 0],
    ['Notte (N)', cIn['N'] || 0],
    ['Riposo (R)', cIn['R'] || 0],
    ['Ferie (F)', cIn['F'] || 0],
  ];
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  ws['!cols'] = [{ wch: 18 }, ...days.map(() => ({ wch: 4 }))];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Inverse');
  const fname = `Inverse_${sanitizeName(opName) || 'operatore'}_${month + 1}_${year}.xlsx`;
  ...
  await FileSystem.writeAsStringAsync(uri, b64, { encoding: FileSystem.EncodingType.Base64 });
  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(uri, { mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', dialogTitle: 'Esporta Inverse in Excel', UTI: 'org.openxmlformats.spreadsheetml.sheet' });
  }
}
