...
import {
  AuditEntry, BuildStats, ProfileConfig, Cell, Coverage, Desiderata, EngineContext, Ferie, GenerationMode, PlanMode, Matrice, Piano, PianoStore, Reparto, Snapshot, Staff, Turno,
} from '../types';
...
import { cloneDeep, isWork, monthKey, migrateFerie, migratePianos, splitAssenzaRange } from '../utils/helpers';
import { recordAudit } from '../services/auditLog';
import { getOutbox } from '../services/syncBootstrap';
import { toSyncOp } from '../services/syncMapper';
...
const initialState: StoreState = {
  ...
  matriciCustom: [],
  matriceMese: {},
  profile: { kind: 'initials', color: '#79B8FF' },
};
...
interface StoreContextValue {
  ...
  profile: ProfileConfig;
  setProfile: (patch: Partial<ProfileConfig>) => void;
  matriciCustom: Matrice[];
  matriceMese: Record<string, string>;
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
      ...
      if (d) {
        dispatch({
          ...
          payload: {
            ...
            profile: d.profile || { kind: 'initials', color: '#79B8FF' },
          },
        });
      } else {
  ...
  }, [state.reparti, state.staff, state.ferie, state.pianos, state.month, state.year, state.audit, state.history, state.future, state.desiderata, state.mode, state.aiMode, state.matriciCustom, state.matriceMese, state.profile, state.hydrated]);
  ...
  const commit = useCallback(
    (patch: Partial<StoreState>, op: string, entity: string, before: string | null, after: string | null, motivo?: string) => {
      ...
      const entry: AuditEntry = { id: genAuditId(), ts: new Date().toISOString(), op, entity, before, after, actor: 'coordinatore', motivo };
      const audit = [entry, ...s.audit].slice(0, AUDIT_MAX);
      merge({ ...patch, history, future: [], audit });
      // Produttore della sync queue (FASE 8): ogni mutazione di dominio genera una operazione offline-first.
      const so = toSyncOp(op, entity);
      if (so) { try { void getOutbox().enqueue(so.entity, so.action, { patch, before, after, motivo, ts: Date.now() }); } catch { /* mai bloccare la UI */ } }
    },
    [merge],
  );
  ...
  const removeReparto = useCallback((id: string) => {
    ...
    recordAudit({ actor: 'coordinatore', role: 'coordinatore', azione: 'rimuovi_reparto', target: removed ? removed.nome : id });
  ...
  const removeStaff = useCallback((id: string) => {
    ...
    recordAudit({ actor: 'coordinatore', role: 'coordinatore', azione: 'rimuovi_operatore', target: removed ? removed.nome : id });
  ...
  const addAssenzaRange = useCallback((infId: string, fromY: number, fromM0: number, fromD: number, toY: number, toM0: number, toD: number, motivo?: string) => {
    const s = stateRef.current;
    const entries = splitAssenzaRange(infId, fromY, fromM0, fromD, toY, toM0, toD, motivo) as Ferie[];
    if (!entries.length) return;
    const ferie = [...s.ferie, ...entries];
    const ctx: EngineContext = { ...ctxOf(s), ferie };
    const { pianos } = rebuild(s, ctx, false);
    commit({ ferie, pianos }, 'create', 'assenza', null, `${entries.length} mesi · ${motivo || 'Assenza'}`);
  }, [commit, rebuild]);
  ...
  const setProfile = useCallback((patch: Partial<ProfileConfig>) => { merge({ profile: { ...stateRef.current.profile, ...patch } }); }, [merge]);
