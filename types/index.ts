...
export interface Notifica {
  id: string;
  mittenteId: string;       // userId/email dello staff che ha generato l'evento
  mittenteNome: string;
  tipo: 'desiderata' | 'disponibilita' | 'ferie' | 'permesso' | 'reperibilita' | 'richiesta';
  testo: string;
  reparto?: string;
  ts: string;               // ISO
  letta: boolean;
...
export interface Staff {
  ...
  family?: FamilyConstraint; // Vincolo Familiare (Inverse Engine): turni del convivente
}

// Vincolo Familiare (Inverse Engine) — vedi services/familyConstraint.ts per la logica.
export type FamilyRelation = 'CONIUGE' | 'PARTNER' | 'GENITORE' | 'ALTRO';
export type FamilyPriority = 'BASSA' | 'MEDIA' | 'ALTA' | 'CRITICA';
export type FamilySource = 'matrix' | 'linked'; // 'matrix' = matrice convivente esterna; 'linked' = altro operatore gestito
export type WeekendRule = 'condivisi' | 'alternati' | 'indifferente';
export type NottiRule = 'alternate' | 'indifferente';
export type GiorniRule = 'insieme' | 'separati' | 'indifferente';
export interface FamilyRules {
  weekend: WeekendRule;   // weekend condivisi (stesso riposo) o alternati
  notti: NottiRule;       // notti alternate fra i due
  giorni: GiorniRule;     // giorni insieme (stesso riposo) o separati (presenza a turno)
}
export interface FamilyConstraint {
  ...
  source?: FamilySource;        // default 'matrix' per retrocompat
  linkedStaffId?: string;       // INVERTED MATRIX: operatore gestito collegato (coniuge/convivente turnista)
  inverseMatrix: Turno[];       // matrice (sequenza turni) del convivente (modalità 'matrix')
  rules?: FamilyRules;          // vincoli configurabili (giorni/weekend/notti)
  notes?: string;
}
...
export type AvatarKind = 'initials' | 'icon' | 'man' | 'woman' | 'emoji' | 'vector';
export interface ProfileConfig {
  kind: AvatarKind;
  color: string;
  initials?: string;   // iniziali personalizzate (override)
  icon?: string;       // chiave icona dalla libreria locale
  emoji?: string;      // glyph emoji locale (legacy, non più offerto in UI)
  vector?: string;     // id avatar vettoriale (libreria SVG)
}
